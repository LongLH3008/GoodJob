...
import { ChevronLeft, ChevronRight, ClipboardList, FilterX, Frown, User } from "lucide-react";
...
const CompanyFilterList = (props: Props) => {
	const { name, location } = CompanySearchState().companySearch;
	const { isChange, resetSearch } = CompanySearchState();
	let filter = `
	${name !== "" ? "&name=" + name : ""}
	...
	useEffect(() => {
		const a = () => {
			setLimit(window.innerWidth < 500 ? "2" : "6");
		};
	...
	const { data } = useSWR(`/company?page=${page}&limit=${limit}${filter}`, SwrFetcher);

	return (
		<div
			className={`${
				isChange ? "block" : "hidden"
			} res_layout bg-gradient-to-b from-zinc-100 from-80% to-white h-screen py-10`}
		>
			<div className="w-full flex justify-between items-center mb-5">
				<p className="text-lg font-normal font-sans my-3">
					Search results ({data?.metadata.total ?? 0})
				</p>
				<span
						...
						<SkeletonProvider
							...
							children={
								<div className="bg-white shadow-sm col-span-1 rounded-lg flex flex-col justify-between items-center p-10 max-sm:p-5 gap-3">
									...
									<div className="w-full grid grid-cols-2 gap-2 *:text-left *:p-2 *:px-3 *:text-primary *:text-[12px] *:bg-zinc-200 *:rounded-md">
										<span className="text-sm flex gap-2">
											<User strokeWidth={1.25} size={16} /> Size:{" "}
											{item.size}
										</span>
										<span className="text-sm flex gap-2">
